...
Learn Linux and cloud operations on a disposable server instead of a quiz. You open a lab, get your own Linux host within a minute or two, fix what is broken from a terminal in the browser, and a grader SSHes into the live machine to check whether you actually fixed it.

Three pieces carry it: a browser-to-SSH terminal, a dedicated lab host per session on AWS, and grading that inspects system state instead of comparing answers.
...
That only holds if the host is genuinely separate. A shared host or a browser-only container would make container isolation the lone wall between a hostile learner and everyone else, so one escape would compromise every session at once. A per-session host spends a visible cost instead, a minute or two of provisioning and a per-session bill, to buy the isolation the security model rests on.
